...
with tab_pipe2:
    ...
    if df_all.empty:
        ...
        if not df_spill.empty:
            ...
            prev_qtr_start = pd.Timestamp(today_ts.year, qtr_start_month, 1) - pd.DateOffset(months=3)
            prev_qtr_end = pd.Timestamp(today_ts.year, qtr_start_month, 1) - pd.Timedelta(days=1)
            ...
            created_prev_qtr = (df_spill["CREATED_ON"] < curr_qtr_start)
            completed_before_qtr = df_spill["ACTUAL_COMPLETED_DATE"].notna() & (df_spill["ACTUAL_COMPLETED_DATE"] < curr_qtr_start)
            cancelled_before_qtr = df_spill["CANCELLED_DATE"].notna() & (df_spill["CANCELLED_DATE"] < curr_qtr_start) & (df_spill["PROJECT_STATUS"] == "Cancelled")
            not_done_before_qtr = ~completed_before_qtr & ~cancelled_before_qtr
            is_primary_spill = (df_spill["UPSELL_DEAL_ATTRIBUTION"] == "Primary") | (df_spill["PRIMARY FULL ACCOUNT?"] == "Primary Full Account")
            spillover_mask = created_prev_qtr & not_done_before_qtr & is_primary_spill
